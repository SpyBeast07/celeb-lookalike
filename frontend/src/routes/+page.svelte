<script lang="ts">
  ...
  async function fetchCelebImage(name: string, age?: number) {
    try {
      ...
      let searchQuery = age ? `${cleanName} ${age} years old portrait` : `${cleanName} portrait`;
      ...
      if (searchData.query && searchData.query.pages) {
        ...
        if (page.title.toLowerCase().includes(cleanName.split(' ')[0].toLowerCase())) return page.thumbnail?.source || null;
      }
      return null;
    } catch (err) {
  ...
  async function handleFind() {
    ...
    try {
      ...
      const apiRes = await fetch('http://localhost:8000/analyze', { method: 'POST', body: formData });
      ...
      if (data.results && data.results.length > 0) {
        ...
        const actorPromises = topFace.matches.slice(0, 5).map(async (m: any) => ({
          name: m.name, confidence: m.confidence, image: await fetchCelebImage(m.name, detectedAge)
        }));
        const cartoonPromises = topFace.matches.slice(0, 5).map(async (m: any) => ({
          name: m.name + " (C)", confidence: m.confidence * 0.95, image: await fetchCelebImage(m.name, detectedAge)
        }));
  ...
  function startProjection() {
    ...
    showOnTV = true;
    projectionStep = 1;
    setTimeout(() => { if (showOnTV) projectionStep = 2; }, 2500);
  }
  ...
  <main class="main-layout">
    <div class="left-column">
      ...
      <VideoPanel title="TV Screen" accent="#00ff88">
        {#if showOnTV && projectedMatch}
          ...
          <div class="tv-projection-container step-{projectionStep}" ondragover={(e) => e.preventDefault()} ondrop={handleDrop}>
            ...
            <div class="projection-part match-side">
              <div class={projectionStep === 1 ? 'avatar-large' : 'avatar-medium'}>
                {#if projectedManualImage || projectedMatch.image}
                  <img src={projectedManualImage || projectedMatch.image} alt={projectedMatch.name} class="avatar-img" />
                {/if}
              </div>
            </div>
          </div>
    ...
    <div class="right-column">
      <div class="controls-row">
        {#if capturedImage}<AppButton variant="secondary" onclick={handleReset}>Reset Cam</AppButton>{/if}
      ...
      <div class="unified-results-grid">
        ...
        {:else}
          {#each actors as actor}
            <ResultCard item={actor} type="actor" selected={selectedMatch === actor} onclick={() => selectMatch(actor)} />
          {/each}
          {#each cartoons as cartoon}
            <ResultCard item={cartoon} type="cartoon" selected={selectedMatch === cartoon} onclick={() => selectMatch(cartoon)} />
          {/each}
        {/if}
        ...
        <div class="settings-card integrated">
          ...
          <div class="card-body-horizontal">
              ...
              {#if selectedMatch}
                <div class="preview-img-box">
                  {#if manualSelectedImage || selectedMatch.image}
                    <img src={manualSelectedImage || selectedMatch.image} alt={selectedMatch.name} class="avatar-img" />
                  {:else}
                ...
                <div class="preview-info">
                  {#if !manualSelectedImage}
                    ...
                    <span class="preview-conf">{Math.round(selectedMatch.confidence * 100)}% Match</span>
                  {:else}
            ...
            <div class="preview-actions">
              <AppButton variant="secondary" onclick={() => document.getElementById('celeb-upload')?.click()} disabled={!selectedMatch} style="flex: 1;">
                CHANGE IMAGE
              </AppButton>
              <input type="file" id="celeb-upload" style="display: none;" accept="image/*" onchange={handleFileSelect} />
              
              <AppButton variant="primary" onclick={startProjection} disabled={!selectedMatch} style="flex: 2; background: var(--accent-color); color: #000;">
                SHOW LIVE
              </AppButton>
            </div>
          </div>
...
<style>
  ...
  .preview-img-box { width: 60px; height: 60px; border-radius: 6px; overflow: hidden; border: 1px solid rgba(255, 255, 255, 0.1); background: #111; }
  .preview-info { display: flex; flex-direction: column; gap: 2px; }
  .preview-name { font-size: 0.9rem; font-weight: 800; color: #fff; }
  .preview-conf { font-size: 0.7rem; color: var(--accent-color); font-weight: 600; }
  .empty-preview { height: 60px; display: flex; align-items: center; justify-content: center; width: 100%; font-size: 0.7rem; opacity: 0.4; }
  .card-header { font-size: 0.75rem; font-weight: 900; letter-spacing: 2px; opacity: 0.5; margin-bottom: 5px; }
  .scanning-indicator { display: flex; flex-direction: column; align-items: center; gap: 15px; }
  .scan-text { font-size: 0.7rem; letter-spacing: 2px; font-weight: 700; }
  .scan-bar { width: 140px; height: 2px; background: var(--accent-color); position: relative; overflow: hidden; }
  .scan-bar::after { content: ''; position: absolute; inset: 0; background: linear-gradient(90deg, transparent, #fff, transparent); animation: scan 1.5s infinite; }
  @keyframes scan { 0% { transform: translateX(-100%); } 100% { transform: translateX(100%); } }
  .label { font-size: 0.55rem; margin-top: 10px; opacity: 0.4; letter-spacing: 1px; font-weight: 700; }
  .name-tag { font-size: 0.9rem; font-weight: 800; margin-top: 10px; color: #fff; transition: all 0.8s cubic-bezier(0.16, 1, 0.3, 1); }
  .name-tag-large { font-size: 1.8rem; font-weight: 900; margin-top: 15px; color: #fff; transition: all 0.8s cubic-bezier(0.16, 1, 0.3, 1); }
